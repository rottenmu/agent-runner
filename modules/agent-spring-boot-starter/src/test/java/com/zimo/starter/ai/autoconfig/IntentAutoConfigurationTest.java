...
import com.zimo.starter.ai.intent.LlmIntentChecker;
import com.zimo.starter.ai.intent.demo.OrderQuerySkill;
...
class IntentAutoConfigurationTest {
    ...
    void routerRoutesWithCollectedSkills() {
        contextRunner.run(context -> {
            IntentAwareSkillRouter router = context.getBean(IntentAwareSkillRouter.class);
            var decision = router.route("帮我查一下订单", com.zimo.starter.ai.intent.ConversationContext.of("t", "u", "s"));
            assertThat(decision.status()).isEqualTo(com.zimo.starter.ai.intent.IntentRoutingDecision.Status.SINGLE);
        });
    }
}
