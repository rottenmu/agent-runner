...
public interface PluginContext {
    ...
    /** 注册动态 Profile 补丁。 */
    void registerProfilePatch(com.zimo.starter.ai.agent.AiProfilePatchProvider patch);

    /** 注册动态 around 中间件（对齐 dsh 事件级瀑布：插件可在请求链任意点包裹/接管）。 */
    void registerMiddleware(com.zimo.starter.ai.agent.AiAgentMiddleware middleware);

    /** 插件事件总线：订阅运行时事件（turn 循环/工具流水线/生命周期）。 */
    com.zimo.starter.ai.plugin.PluginEventBus eventBus();
