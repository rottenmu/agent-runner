...
public class AiAgentService {
    ...
    private final List<com.zimo.starter.ai.agent.AiRequestInterceptor> requestInterceptors;
    private final List<com.zimo.starter.ai.agent.AiAgentMiddleware> middlewares;
    /** 插件事件总线（可空）：非空时派发 turn begin/end 事件（dsh A1 事件级瀑布）。 */
    private final com.zimo.starter.ai.plugin.PluginEventBus pluginEventBus;
    ...
    public AiAgentService(
            ...
            List<com.zimo.starter.ai.agent.AiRequestInterceptor> requestInterceptors,
            List<com.zimo.starter.ai.agent.AiAgentMiddleware> middlewares) {
    ...
    public AiAgentService(
            ...
            List<com.zimo.starter.ai.agent.AiRequestInterceptor> requestInterceptors,
            List<com.zimo.starter.ai.agent.AiAgentMiddleware> middlewares,
            com.zimo.starter.ai.plugin.PluginEventBus pluginEventBus) {
    ...
    private AiAgentReply invokeHarnessWithHistory(
            ...
            AiAgentProfile profile,
            List<Msg> history) {
        com.zimo.starter.ai.agent.AiRequestContext context =
                com.zimo.starter.ai.agent.AiRequestContext.of(
                        routeRequest.conversationId(), profile.id(), profile);
        com.zimo.starter.ai.agent.MiddlewareChain terminal =
                (ctx, msg) -> executeCoreWithHistory(ctx, msg, routeRequest, profile, history);
        com.zimo.starter.ai.agent.MiddlewareChain chain =
                com.zimo.starter.ai.agent.AiAgentMiddleware.buildChain(middlewares, terminal);
        com.zimo.starter.ai.agent.AiMiddlewareResult result = chain.proceed(context, message);
        switch (result.kind()) {
            case DENY:
    ...
    private com.zimo.starter.ai.agent.AiMiddlewareResult executeCoreWithHistory(
            com.zimo.starter.ai.agent.AiRequestContext context,
            String message,
            AiAgentRouteRequest routeRequest,
            AiAgentProfile profile,
        ...
        if (intercepted.denied()) {
            return com.zimo.starter.ai.agent.AiMiddlewareResult.deny(
                    "请求被拦截：" + intercepted.deniedReason());
        }
        message = intercepted.message();
        long start = System.currentTimeMillis();
        String traceId = com.zimo.starter.ai.observ.TraceCollector.begin(
        ...
        try {
            ...
            com.zimo.starter.ai.observ.TraceCollector.step("intent", "意图路由",
            ...
            if (response == null || !hasText(response.getTextContent())) {
                ...
                com.zimo.starter.ai.observ.TraceCollector.end("failed", message, failMsg, 0,
                        System.currentTimeMillis() - start);
                return com.zimo.starter.ai.agent.AiMiddlewareResult.reply(
                        new AiAgentReply(agentName(profile), failMsg));
            }
            String content = response.getTextContent();
            com.zimo.starter.ai.observ.TraceCollector.step("generation", "智能体回复",
                    "{\"prompt\":\"" + safeJson(message) + "\"}",
                    "{\"response\":\"" + safeJson(content) + "\"}", System.currentTimeMillis() - start, "ok");
            com.zimo.starter.ai.observ.TraceCollector.end("ok", message, content, estimateTokens(content),
                    System.currentTimeMillis() - start);
            return com.zimo.starter.ai.agent.AiMiddlewareResult.reply(
    ...
    private AiAgentReply invokeHarness(
        ...
        if (pluginEventBus != null) {
            pluginEventBus.emit(com.zimo.starter.ai.plugin.PluginEventBus.AGENT_TURN_BEGIN,
                    java.util.Map.of(
        ...
        com.zimo.starter.ai.agent.AiMiddlewareResult result = chain.proceed(context, message);
        if (pluginEventBus != null) {
            pluginEventBus.emit(com.zimo.starter.ai.plugin.PluginEventBus.AGENT_TURN_END,
                    java.util.Map.of(
                            "conversationId", routeRequest.conversationId() == null ? "" : routeRequest.conversationId(),
                            "agentId", profile.id(),
    ...
    private com.zimo.starter.ai.agent.AiMiddlewareResult executeCore(
            com.zimo.starter.ai.agent.AiRequestContext context,
            String message,
            AiAgentRouteRequest routeRequest,
            AiAgentProfile profile) {
    ...
    private InterceptResult runInterceptors(String sessionId, String agentId, AiAgentProfile profile, String message) {
        ...
        for (com.zimo.starter.ai.agent.AiRequestInterceptor interceptor : requestInterceptors) {
            com.zimo.starter.ai.agent.AiRequestDecision decision =
                    interceptor.intercept(com.zimo.starter.ai.agent.AiRequestContext.of(sessionId, agentId, profile), current);
            if (decision == null || decision.action() == com.zimo.starter.ai.agent.AiRequestDecision.Action.PASS) {
                continue;
            }
            if (decision.action() == com.zimo.starter.ai.agent.AiRequestDecision.Action.DENY) {
